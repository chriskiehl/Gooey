...
class TestProcessor(unittest.TestCase):
    ...
    def test_extract_progress(self):
        ...
        processor = ProcessController("^progress: (\d+)%$", None, 'utf-8', False)
        self.assertEqual(processor._extract_progress(b'progress: 50%'), 50)

        processor = ProcessController("total: (\d+)%$", None,  'utf-8', False)
    ...
    def test_extract_progress_returns_none_if_no_regex_supplied(self):
        processor = ProcessController(None, None, 'utf-8', False)
    ...
    def test_extract_progress_returns_none_if_no_match_found(self):
        processor = ProcessController(r'(\d+)%$', None, 'utf-8', False)
    ...
    def test_eval_progress(self):
        ...
        regex = r'(\d+)/(\d+)$'
        processor = ProcessController(regex, r'x[0] / x[1]', 'utf-8', False)
